...
import { ArrowLeft, Clock3, Mail, MapPin, PhoneCall, ShieldAlert, Stethoscope, UserRound } from 'lucide-react'
import Link from 'next/link'

import { allSecurityLocations, emergencySecurityLocations, healthLocations, securityLocations, type SecurityLocation } from '@/lib/seguranca-map'
...
export default function SegurancaPage() {
  const [selectedLocationId, setSelectedLocationId] = useState<string | null>(allSecurityLocations[0]?.id ?? null)

  const selectedLocation = useMemo(() => {
    if (!selectedLocationId) return allSecurityLocations[0] ?? null
    return allSecurityLocations.find((location) => location.id === selectedLocationId) ?? allSecurityLocations[0] ?? null
  }, [selectedLocationId])
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 lg:px-6">
        <header className="rounded-[2rem] border border-border/70 bg-background/92 p-4 shadow-[0_20px_60px_-40px_rgba(15,23,42,0.35)] backdrop-blur-xl sm:p-5">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h1 className="mt-3 text-2xl font-semibold tracking-tight text-foreground sm:text-3xl">Mapa de segurança do Centro de Aracaju</h1>
              <p className="mt-2 max-w-3xl text-sm text-muted-foreground sm:text-base">
                Encontre Polícia Militar, Bombeiros, SAMU, Defesa Civil e Guarda Municipal com contato rápido para ligação, e-mail e referência de responsável.
              </p>
            </div>
          </div>
        </header>
        ...
        <section className="rounded-[2rem] border border-border/70 bg-background/92 p-4 shadow-[0_20px_60px_-40px_rgba(15,23,42,0.35)] backdrop-blur-xl sm:p-5">
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-xl font-semibold text-foreground">Mapa de segurança</h2>
              <p className="text-sm text-muted-foreground">Toque em um marcador para destacar a unidade e ver a rota aproximada a partir da sua localização.</p>
            </div>
            {selectedLocation ? (
          ...
          <SecurityMap
            locations={allSecurityLocations}
        ...
        <section className="space-y-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Lista completa dos cinco serviços principais de resposta e apoio no Centro.</p>
          </div>
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-xl font-semibold text-foreground">Saúde</h2>
              <p className="text-sm text-muted-foreground">Hospitais, clínicas e pontos de atendimento médico adicionados ao mapa de segurança.</p>
            </div>
          </div>
