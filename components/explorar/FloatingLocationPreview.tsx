...
import { ArrowUpRight, Clock3, Route, Sparkles } from 'lucide-react'
...
export default function FloatingLocationPreview({ location, onOpenDetails, onOpenRoute }: FloatingLocationPreviewProps) {
  return (
    <motion.div
    ...
    >
      <div className="w-full rounded-[1.7rem] border border-white/55 bg-background/92 p-4 text-left shadow-[0_24px_70px_-30px_rgba(15,23,42,0.72)] backdrop-blur-xl">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          <button
            type="button"
            onClick={onOpenRoute}
            className="inline-flex items-center justify-center gap-2 rounded-full border border-primary/15 bg-primary/10 px-4 py-2 text-sm font-semibold text-primary transition-colors hover:bg-primary/15"
          >
            Ir
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-full border border-primary/20 bg-background px-4 py-2 text-sm font-semibold text-primary transition-colors hover:bg-primary/5"
          >
            Ver detalhes
